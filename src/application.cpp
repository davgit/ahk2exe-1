...
enum ResultType {FAIL = 0, OK, WARN = OK, CRITICAL_ERROR, CONDITION_TRUE, CONDITION_FALSE 
	, LOOP_BREAK, LOOP_CONTINUE  // added for #include wildcards N12
	, EARLY_RETURN, EARLY_EXIT}; // FAIL must be 0.
...
// adapted from line::fileisfilteredout() in autohotkey
bool FileIsFilteredOut(WIN32_FIND_DATA &aCurrentFile, FileLoopModeType aFileLoopMode
...
// Forward declaration to allow mutual recursion between isincludedirective, includefiles, and loadincludedfile:
...
ResultType IncludeFiles(FILE *aTarget, HWND aStatusBar, HS_EXEArc_Write &oWrite
	, char *aFileSpec, bool aAllowDuplicateInclude, bool aIgnoreLoadFailure, FileLoopModeType aFileLoopMode, bool aRecurseSubfolders)
// adapted from Line::PerformLoopFilePattern to allow wildcards in #Include directives: N11
{
   // Make a local copy of the path given in aFileSpec because as the lines of
   // the loop are executed, the deref buffer (which is what aFileSpec might
   ...
   tcslcpy(file_path, aFileSpec, _countof(file_path));
   ...
   HANDLE file_search = FindFirstFile(aFileSpec, &new_current_file);
...
// N12 don't think we need this here:  global_struct &g = *::g; // Primarily for performance in this case.
	int mLoopIteration = 0 ;
   for (; file_found; ++mLoopIteration)
   {
      // N12 don't think we need this here: g.mLoopFile = &new_current_file; // inner file-loop's file takes precedence over any outer file-loop's.
      ...
      if (!LoadIncludedFile(aTarget, aStatusBar, oWrite, new_current_file.cFileName, aAllowDuplicateInclude, aIgnoreLoadFailure)) // N12 loadincludedfile in ahk2exe is different from autohotkey
         {
   ...
   // search for more files and folders inside that match aFileSpec.  We can't do this in the
   ...
   // Append *.* to file_path so that we can retrieve all files and folders in the aFileSpec
...
// #y::run, notepad
{
    ...
    */
		 // N11 added option to recurse into subdirectories
...
cp = parameter ; // ahk2exe and autohotkey use different variable names...  N12
		...
		*escaped_comment_flag = g_EscapeChar;
		...
		return IncludeFiles(aTarget, aStatusBar, oWrite, filename, include_again, ignore_load_failure, loopmode, recurse) ? CONDITION_TRUE : FAIL;
