...
/* 
	10th Sept 2003 - Jonathan Bennett <jon@hiddensoft.com>
	This source was taken from the NSIS installer project at
	http://nsis.sourceforge.net

	The only change made was moving the constructor into a new Init()
	function as I didn't want to use the c++ throw() exceptions
*/
...
bool CResourceEditor::Init(BYTE* pbPE, int iSize) {
  ...
  if (m_dosHeader->e_magic != IMAGE_DOS_SIGNATURE)
    return false; //throw runtime_error("PE file contains invalid DOS header");

  // Get NT headers
  m_ntHeaders = (PIMAGE_NT_HEADERS)(m_pbPE + m_dosHeader->e_lfanew);
  if (m_ntHeaders->Signature != IMAGE_NT_SIGNATURE)
    return false; //throw runtime_error("PE file missing NT signature");
  ...
  for (int i = 0; i < m_ntHeaders->FileHeader.NumberOfSections; i++) {
    if (m_dwResourceSectionVA == sectionHeadersArray[i].VirtualAddress) {
      ...
      if (!sectionHeadersArray[i].PointerToRawData)
        return false; //throw runtime_error("Invalid resource section pointer");
    }

    // Invalid section pointer (goes beyond the PE image)
    if (sectionHeadersArray[i].PointerToRawData > (unsigned int)m_iSize)
      return false; //throw runtime_error("Invalid section pointer");
  }

  // No resource section...
  if (m_dwResourceSectionIndex == m_ntHeaders->FileHeader.NumberOfSections)
    return false; //throw runtime_error("PE file doesn't contain any resource section");
